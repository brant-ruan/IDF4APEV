...
{
    time_t now = time(0);
    unsigned target_count = 100;
    char *key_prefix = "rambo_";
    char key_name[512] = {0};
    sprintf(key_name, "%s%ld", key_prefix, now);
    int i = 0;
    key_serial_t serial;

    serial = syscall(__NR_KEYCTL, KEYCTL_JOIN_SESSION_KEYRING, key_name);
    if (serial < 0)
    {
        perror("sys_keyctl");
        return ERROR;
    ...
    {
        sscanf(res, "%*s%*s%*s%*s%*s%*s%*s%*s %[^:]", temp_name);
        if (!strcmp(temp_name, key_name))
        {
            //printf("%s\n", res);
            fclose(fp);
            sscanf(res, "%*s%*s %u", &usage);
            if (target_count + 1 == usage)
                return VULNERABLE;
            else
